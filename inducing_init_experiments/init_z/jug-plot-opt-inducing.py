...
name_dict = {"Kmeans":"K-means (reinit.)","Greedy Conditional Variance":"Greedy var. (reinit.)","Gradient":"Gradient"}
plot_title_dict = {"Wilson_energy":"Energy (M=65)", "Wilson_elevators":"Elevators (M=1200)","Naval_noisy":"Naval with Noise (M=55)"}
...
for dataset in init_Z_runs.keys():
    ...
    for init_Z_method in init_Z_runs[dataset].keys():
        ...
        l, = ax.plot(np.arange(len(best_elbo[0])), best_elbo[0], label=name_dict[init_Z_method],color=color_dict[init_Z_method])
